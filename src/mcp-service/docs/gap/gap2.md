...
> **Gap 2 gần đóng** — chỉ còn 2.6 ở phạm vi hr/mcp.
> - 🟢 **Đã đóng:** 2.1, 2.2, 2.3, 2.8 (`c52c258`+`85e0926`); **2.4, 2.5, 2.7** (self-access + 7 intent + publish enum, đợt này).
> - 🔴 **Còn mở (không cần SA):** 2.6 (tách `verify()` khỏi fail-closed — đụng `main.py`, làm PR riêng).
...
> - ⏸️ **Hoãn:** `recruitment` (cross-user → đợt hierarchy).
>
> → Phía hr-service + mcp-service: đóng Gap 2 khi xong **2.6**. Phần native/query-service là việc của service khác.
...
| 2.6 | Tách verify khỏi fail-closed | Không | TB | 🔴 còn lại duy nhất ở hr/mcp |
