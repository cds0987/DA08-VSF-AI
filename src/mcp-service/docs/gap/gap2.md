...
- Thêm hằng `MVP_INTENTS` + `HrIntent` (Literal) đầu file, khớp `Literal` hr-service ([routes.py](../../../hr-service/app/api/routes.py)). *(Cập nhật: tập intent sau này mở rộng lên 7 — xem 2.4/2.5.)*
...
- ⚠️ Intent ngoài tập (vd `recruitment` — cross-user, chưa hỗ trợ) → rơi vào nhánh này, nhận message sạch thay vì 422.
- **Test thêm:** `test_proxy_rejects_unknown_intent` — gọi `fn(USER_HR, "recruitment")`, assert không có call POST nào (`client.calls == []`) và `result["summary"]` chứa thông báo.
