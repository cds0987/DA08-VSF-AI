...
> Trạng thái: 🟢 Đã chạy ở mcp-service qua HTTP proxy sang hr-service với **7 intent**: `leave_balance` / `leave_requests` / `attendance` / `onboarding` / `payroll` / `benefits` / `performance` (3 cái sau = self-access + audit, không role-gate). Các phần chưa hoàn thiện xem ở [`docs/gap/gap2.md`](../gap/gap2.md).

> 📝 `hr_query` chỉ là **READ**. Luồng **WRITE** (tạo đơn nghỉ phép) là tool MCP riêng — xem [`create_leave_request.md`](./create_leave_request.md).
...
| SA-3 | **Nguồn role cho intent nhạy cảm** — JWT claim nào query-service truyền để gate `payroll` / `recruitment`. | ✅ **RESOLVED** — chốt **self-access** (chỉ data của chính user, lọc `WHERE user_id`): `payroll`/`benefits`/`performance` **không cần role-gate**, chỉ audit log. `recruitment` hoãn (cross-user). | B6 (bỏ — không gate) |
...
**Giai đoạn 1 — MVP:** `leave_balance` + `leave_requests` + `attendance` + `onboarding`. ✅ DONE.
- Không cần role-gate (độ nhạy Thấp/Trung bình).
- Intent ngoài danh sách → soft-fail (`_unsupported_intent_response`), không lộ data.

**Giai đoạn 2 — mở rộng:** `payroll`, `benefits`, `performance`. ✅ DONE (self-access).
- SA-3 chốt **self-access** → không role-gate; chỉ **audit log** mỗi lần truy cập (`SENSITIVE_INTENTS` ở hr-service `routes.py`).
- `Literal HrIntent` ở [hr_query.py](../../app/tools/hr_query.py) + `routes.py` giờ liệt kê **đủ 7 intent**.
- `recruitment` **hoãn** (cross-user, không hợp self-access). `employee_profile`/`org_structure` lấy từ JWT claim (SA-2), không tạo bảng.
...
- **Đã có tool ở mcp-service.** `hr_query` đã đăng ký và hoạt động như HTTP proxy sang hr-service cho **đủ 7 intent**; e2e CI (`e2e_hr_integration.py`) chứng minh chain mcp→hr→Postgres. Các gap còn lại được theo dõi tại [`docs/gap/gap2.md`](../gap/gap2.md).
- Server **đã expose** tool `hr_query` + publish enum 7 intent qua interface MCP (`list_tools()`/schema). Theo nguyên tắc ranh giới, client (query-service…) chỉ tương tác qua interface này và discover đủ 7 intent — cách client nội bộ ánh xạ intent là việc của client, không thuộc phạm vi mcp/hr.
