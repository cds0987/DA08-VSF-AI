...
# Phiên bản thuật toán SPARSE (BM25). PHẢI khớp rag-worker/core_engine/contract.py.
...
def index_id(collection: str, model: str, dimension: int, *, sparse_version: int = 0) -> str:
    ...
    # sparse_version>0 (hybrid BM25) -> hậu tố __s{ver}; =0 -> giữ nguyên tên cũ.
...
def vectorstore_fingerprint(
    *,
    ...
    # Chỉ thêm key khi >0 -> fingerprint dense-only GIỮ NGUYÊN => không vỡ stamp prod.
