...
```bash
cd src/rag-service
.venv\Scripts\python -m pip install -r core_engine/requirements.txt

.venv\Scripts\python -m core_engine.demo                    # demo end-to-end
.venv\Scripts\python -m core_engine.tests.selftest          # assert bất biến (offline)
.venv\Scripts\python -m core_engine.tests.selftest_provider # AI gateway: SDK wiring + e2e
.venv\Scripts\python -m core_engine.tests.selftest_async    # async qua các stage (concurrency/to_thread/retry)
.venv\Scripts\python -m core_engine.tests.selftest_vectorstore # chọn backend + CONTRACT chung cho mọi DB
```
...
```python
from core_engine.ai import get_ai_provider
...
```python
from core_engine import build_engine, VectorStoreConfig
from core_engine.vectorstore import build_vector_store, register_backend
...
```python
from core_engine import build_engine_probe
