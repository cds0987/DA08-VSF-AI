...
> **Cập nhật gần nhất: 2026-06-03** sau loạt commit `bc0d502 → b75f374` (enforce contract,
...
| 18 | Metadata DB + job log + retention/prune | ingestion §8,10 | ✅ **Phần lớn** (90fbc2a + 0b7f4f1): `DocumentRepository` (InMemory + Postgres), **Alembic migration** (bỏ create_all ad-hoc), index `created_at`, `update_chunk_count` trên port, status PROCESSING/COMPLETED/FAILED. **Còn** job-log table + retention/prune job | 🟡 |
...
| 22 | Config validation startup (provider+url+model, model+dim+index) | ingestion §10 | 🟡 **Một phần** (45b28df): `runtime` validate `EMBED_DIMENSION`/`SEARCH_TOP_K`/`RERANK_TOP_K`/provider/collection/dimension + remote cần url. **Còn** cặp `model↔dim↔index id` và `backend↔credential` | 🟡 |
...
- **#18 Metadata DB — còn job-log + retention.** Đã có repo + alembic migration + index `created_at`;
  còn **bảng job-log** + **retention/prune job** (DAY0 §14) và atomic-claim cho status (gắn #13).
- **#21 Eval gate.** Có notebook prototype, chưa thành gate chặn merge (golden queries + expected
  lineage + recall/precision/no-answer + p50/p95). Goal coi đây là ràng buộc cứng cho mọi thay đổi
  parser/caption/model/index/rerank.
- **#22 Config validation chưa đủ.** `runtime` đã validate dimension/top_k/provider/collection;
  còn cặp `model↔dimension↔index id` và `backend↔credential`.
- **#26 Contract test chưa chạy backend thật.** Mới `:memory:`; cần chạy remote Qdrant trong CI
  (lesson "in-memory không chứng minh production semantics").
...
5. **Job-log + retention (#18 còn lại)** + **cost guardrail (#24)**.
6. **Ratify ★ rồi implement:** change detector (#17), coalescer (#19), hybrid (#20), versioning (#27), deploy (#25).

> Đã đóng (không cần làm lại): #7, #8, #9, #10, #11, #12, #14 (prune), #18 (repo+migration), #23.
