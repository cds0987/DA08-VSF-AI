...
| 18 | Metadata DB + job log + retention/prune | ingestion §8,10 | ✅ **Phần lớn**: `DocumentRepository` (InMemory + Postgres), **Alembic migration**, index `created_at`, `update_chunk_count`, **job-log table + append/list/prune API** đã có. **Còn** retention/prune job theo lịch và lifecycle policy | 🟡 |
...
| 22 | Config validation startup (provider+url+model, model+dim+index) | ingestion §10 | 🟡 **Phần lớn**: `runtime` validate `EMBED_DIMENSION`/`SEARCH_TOP_K`/`RERANK_TOP_K`/split bounds/provider/collection naming/remote url; AI provider thật đã fail-fast ở `provider.validate()`. **Còn** credential policy cho remote vector backend và các ràng buộc backend-specific khác | 🟡 |
...
- **#18 Metadata DB — còn retention job/lifecycle.** Đã có repo + alembic migration + index `created_at`
  + **bảng job-log / append-list-prune API**; còn **retention/prune job theo lịch** (DAY0 §14) và
  atomic-claim cho status (gắn #13).
...
- **#22 Config validation chưa đủ.** `runtime` đã validate bounds/provider/collection naming/remote url;
  AI provider thật đã fail-fast ở startup. Còn thiếu **credential policy cho remote vector backend**
  và các rule backend-specific sâu hơn.
