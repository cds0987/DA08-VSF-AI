...
**doc.status hiện là at-least-once trong rag-worker.**
Worker vẫn publish inline ngay sau khi job terminal xong. Nếu publish trượt sau 3 lần retry,
job vẫn giữ `status_published_at = NULL`; background sweep định kỳ quét mọi job terminal chưa
đánh dấu phát thành công rồi publish lại. Vì cờ nằm trong DB metadata, cơ chế này sống qua restart
và đóng cả hai lỗ: `COMPLETED/FAILED nhưng mất tín hiệu` (G7-8/G7-16). Reconciler bucket vẫn giữ
vai trò riêng cho lỗ `no-row` khi rag-worker chưa từng biết document.

**Reconcile flow** (opt-in):
```text
bucket raw/<doc_id>/<file>
  -> store reconciler list_objects_v2 định kỳ
  -> parse key -> tra documents table
  -> không có row nào: enqueue lại ingest qua use-case nội bộ
  -> có row bất kỳ (queued/processing/completed/failed/deleted): skip
```
Cost guard: mặc định tắt; khi bật phải cấu hình bucket + interval dương + min-age để tránh đua
với `doc.ingest` đang bay.
...
- **doc.status at-least-once, không exactly-once.** Cơ chế hiện tại là `status_published_at`
  + sweep retry; document-service phải tiếp tục giữ idempotent cho payload trùng. Nếu sau này
  cần exactly-once hoặc audit riêng cho publish transport, mới tách outbox đầy đủ.
...
- Chưa làm: `rag.search` request-reply qua NATS (search vẫn HTTP theo yêu cầu hiện tại).
- Đã làm safety-net change detector ở mức scanner bucket opt-in: store reconciler list bucket
  và enqueue lại object bị sót.
- Đã làm `doc.status` outbox-lite: `status_published_at` + sweep nền đảm bảo at-least-once.
