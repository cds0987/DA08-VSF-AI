...
**doc.status là best-effort optimization, không còn là nguồn duy nhất đảm bảo eventual ingest.**
`publish_for_job` nuốt lỗi publish (retry 3 lần rồi chỉ log) để không làm sập worker. Nếu NATS
chớp tắt đúng lúc publish → BE có thể **mất tín hiệu** indexed/failed ở thời điểm đó, nhưng
`STORE_RECONCILE_ENABLED=1` cho phép rag-worker quét bucket định kỳ và enqueue lại mọi object
`raw/<doc_id>/<file>` mà metadata store chưa từng biết. Điều này hạ G7-8 / G7-16 từ "mất vĩnh viễn"
xuống "trễ tối đa một chu kỳ quét". Reconciler là additive-only, không tự xóa gì; xóa vẫn đi qua
`doc.access{deleted:true}` + tombstone `documents.status=deleted`.
...
- **doc.status best-effort → outbox.** Publish lỗi hiện chỉ log (best-effort), trong
  khi ingest durable → bất đối xứng độ tin cậy. Store reconciler đã đóng lỗ "mất hẳn ingest",
  nhưng **không** thay thế nhu cầu outbox nếu BE phụ thuộc tuyệt đối vào handshake `doc.status`
  tức thời.
...
- Đã làm safety-net change detector ở mức scanner bucket opt-in: store reconciler list bucket
  và enqueue lại object bị sót. Vẫn chưa có outbox publish cho `doc.status`.
