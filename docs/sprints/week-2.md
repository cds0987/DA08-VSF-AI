...
| **Frontend Dev** | Đặng Hồ Hải | chat SSE chạy thật (`StreamingText.vue` đọc token từ `POST /query`); `pages/admin/documents.vue` (`FileUpload` + bảng status); composables `useApi`/`useAuth`/`useChat`. | SSE `/query` (AI Eng) |
| **DevOps** | Trần Hữu Gia Huy | `docker-compose up` full **11 containers** chạy local; dựng **Langfuse server** (:3100) + cấp API key qua Secrets Manager. | Dockerfile các service (T1) |
