...
| OCR cho PDF scan | ⚠️ | ✅ | ❌ | ❌ | ❌ | ✅ (Azure Document Intelligence) |
...
**→ Giải pháp: Azure Document Intelligence, chất lượng OCR cao, hỗ trợ tốt tiếng Việt và bảng/layout phức tạp.**
...
**→ Giải pháp: Self-hosted, open-source stack (FastAPI + Qdrant + PostgreSQL), chỉ trả tiền API (Azure OpenAI + Azure Document Intelligence).**
