...
query-service  ──────────────────────────────────────────►  mcp-service
(MCP client)        rag_search(query, document_ids, top_k)   (MCP tool server, :8003)
                                                                   │
   ◄───────────────────────────────────────────────────────       │ chỉ ĐỌC
        List[SearchResult]  (đã rerank, top-k)                     ▼
                                                                Qdrant
                                                          (rag-worker ghi vào)
```

- **query-service**: gọi `rag_search`, có thể truyền `document_ids` xuống. Tham số nhạy cảm do client inject, **không tin LLM**.
- **mcp-service**: service độc lập, chỉ-đọc. Embed query → retrieve Qdrant → rerank → trả top-k. Ghép với rag-worker **chỉ qua Qdrant** (point_id + payload keys phải khớp cái rag-worker ghi).
...
|-----------|-------|----------|
| query-service | `MockMCPClient` đầy đủ; luồng orchestration (ACL→cache→search→threshold→top-3→LLM); test | **Real MCP client** trỏ tới :8003; `MCP_MODE` switch; đổi field `s3`→`gcs` |
| mcp-service | ✅ Tool `rag_search` thật; **LLM reranker + fallback**; endpoint `/mcp` công bố; fail-closed; README runtime contract; e2e CI xanh | — (xong phần thuộc mcp) |
| docs | contract shape `SearchResult`; file này; README mcp-service | `contracts.md` còn mô tả mcp-service sai so với code thật (cần SA) |
...
- **rag-worker → mcp-service qua Qdrant đã chứng minh end-to-end trên CI** (full validation corpus, mục 11).

### Blocker còn lại 🔴 (đều ở query-service)
1. **Field URI lệch tên** — query-service dùng `*_s3_uri`, mcp-service trả `*_gcs_uri`. **Đã chốt `gcs`** (mục 4) → query-service đổi `s3`→`gcs` (6.3).
2. **Chưa có real MCP client** trong query-service (6.1 + 6.2).
...
mcp-service là **consumer chỉ-đọc Qdrant**, **không bao giờ tải file**:
- [`vectorstore.py`](../src/mcp-service/app/core/vectorstore.py) chỉ đọc string URI từ payload (`source_uri`→`source_gcs_uri`, `artifact_uri`→`markdown_gcs_uri`) rồi **truyền thẳng** ra cho FE cite nguồn — KHÔNG fetch.
...
## 6. Việc cho **query-service**

### 6.1. 🔴 Viết Real MCP client
...
### 6.2. 🔴 `MCP_MODE` switch (mock ↔ real)
- File: [`app/infrastructure/config.py`](../src/query-service/app/infrastructure/config.py), [`app/interfaces/api/dependencies.py`](../src/query-service/app/interfaces/api/dependencies.py) (`get_mcp_client` hiện hard-code `MockMCPClient`).
- Thêm env `MCP_MODE=mock|real` + `MCP_SERVICE_URL`. `real` → trả real client.
- Cập nhật `/health` báo `mcp_service=real|mock` + ping được mcp-service.

### 6.3. 🔴 Đổi field URI `s3` → `gcs` (đã chốt mục 4)
- Đổi `source_s3_uri`/`markdown_s3_uri` → `source_gcs_uri`/`markdown_gcs_uri` ở:
...
## 7. Thứ tự thực hiện — còn lại (đều ở query-service)

1. ✅ ~~Field URI chốt `gcs`~~ + ✅ ~~mcp-service: LLM reranker, document_ids, endpoint, fail-closed~~ (xong).
2. **query-service: đổi field `s3`→`gcs`** (6.3) — đổi tên ở 5 chỗ.
3. **query-service: real MCP client** (6.1) + **MCP_MODE switch** (6.2) → gọi `/mcp` :8003.
4. Rà contract nhỏ (6.4), cập nhật `/health`.
5. **SA: cập nhật `contracts.md`** cho khớp code mcp thật (mục 8).
...
- [x] mcp-service trả field `*_gcs_uri`; **query-service đổi `s3`→`gcs`** thì xong (còn query-service).
- [x] `rag_search` nhận `document_ids` (`None`/`[]`/list dài) không crash; comment/log nói rõ ACL ngoài phạm vi.
- [ ] query-service có real MCP client + `MCP_MODE=real` gọi được :8003.
- [ ] `/health` query-service báo `mcp_service=real` và ping OK.
- [x] E2E rag-worker→mcp qua Qdrant (full corpus) xanh trên CI — mục 11.
- [ ] `contracts.md` khớp code thật (README mcp ✅ xong).
- [ ] Ngưỡng score + cắt top-k không bị áp 2 lần (query-service, 6.4).
...
## 11. E2E CI: rag-worker → mcp-service qua Qdrant (đã xanh)
...
- Service ở giữa **mock bằng message/script**; hạ tầng (Qdrant, MinIO, NATS) **thật**. Ranh giới 2 service duy nhất là **Qdrant**.
...
- Guard kèm: job `contract` (parity fingerprint rag-worker↔mcp) chặn lệch vectorstore contract ngay ở PR.
