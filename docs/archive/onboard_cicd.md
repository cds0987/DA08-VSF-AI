...
# 5 DB: user_db / doc_db / query_db / rag_db / hr_db (pass: Spektrem@1)
```
...
| Postgres | **in-compose** `app-postgres` (KHÔNG Cloud SQL) — 5 DB, superuser `postgres` / pass `Spektrem@1` (URL-encode `%401`), host nội bộ `app-postgres:5432`, volume `app_pg_data` |
