...
#
# CI (deploy.yml) auth bằng Workload Identity Federation -> impersonate
...
#
# LƯU Ý bootstrap: git fetch/reset main được làm Ở PAYLOAD (CI) TRƯỚC khi gọi file
# này — để lần cutover đầu tiên (khi VM chưa có remote-entry.sh) vẫn chạy được. Tới đây
# code đã = origin/main, nên file này chỉ render secret + deploy + seed.
set -euo pipefail
