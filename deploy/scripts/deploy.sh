...
docker compose up -d --no-build --scale rag-ingest-worker=6 qdrant langfuse-db langfuse nats-bootstrap query-migrate rag-worker rag-ingest-worker mcp-service hr-service user-service gotenberg document-service query-service ai-router frontend-chat frontend-admin \
